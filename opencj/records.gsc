...
onInit()
{
	...
	openCJ\commands_base::registerCommand("playback", "Play back a recording", ::_onCommandPlayback, 1, 1, 0);
}
...
whileAlive()
{
	...
	if(isDefined(self.playback))
	{
		...
		self.linker.origin = self readPlaybackFrame_origin();
		self setPlayerAngles(self readPlaybackFrame_angles());
